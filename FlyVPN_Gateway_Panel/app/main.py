...
def login_get(request: Request):
    return tpl.TemplateResponse(request, "login.html", ctx(request))
...
def users_create(
    ...
    xray.write_config()
    flash(request, "Пользователь создан. Если выбрано «Все локации», подписка отдаст все страны отдельными VLESS-ссылками.")
...
def users_update(
    ...
    upid = int(upstream_id) if upstream_id.strip().isdigit() else None
    db.execute(
        "UPDATE users SET username=?, upstream_id=?, active=?, expires_at=?, notes=? WHERE id=?",
        (username, upid, 1 if active == "1" else 0, db.iso(exp), notes, uid),
    )
    xray.write_config()
...
def upstreams(request: Request):
    require_login(request)
    return tpl.TemplateResponse(request, "upstreams.html", ctx(request, upstreams=db.rows("SELECT * FROM upstreams ORDER BY id DESC")))
...
async def upstream_import(request: Request, name: str = Form(""), json_text: str = Form(""), file: UploadFile | None = File(None)):
    ...
    db.execute(
        ...
        (nm, remark, 1, json.dumps(data, ensure_ascii=False, indent=2), cnt, db.iso(db.utcnow())),
    )
    xray.write_config()
...
def xray_page(request: Request):
    ...
    return tpl.TemplateResponse(request, "xray.html", ctx(request, cfg=json.dumps(cfg, ensure_ascii=False, indent=2)))
...
def xray_validate(request: Request):
    ...
    flash(request, ("✅ Xray config валиден" if ok else "❌ Ошибка Xray: ") + out[:1500], "ok" if ok else "bad")
    return RedirectResponse("/xray", status_code=303)
...
def xray_restart(request: Request):
    ...
    flash(request, ("✅ Xray перезапущен" if ok else "❌ Не удалось перезапустить Xray: ") + out[:1500], "ok" if ok else "bad")
    return RedirectResponse("/xray", status_code=303)
...
def settings_get(request: Request):
    require_login(request)
    return tpl.TemplateResponse(request, "settings.html", ctx(request))
